  ...
  return (
    <div className="mx-auto flex min-h-[calc(100vh-72px)] w-full max-w-6xl items-center px-4 pb-28 pt-8 sm:px-7 lg:px-12">
      <section className="w-full overflow-hidden rounded-[34px] border border-[#d7e1d8] bg-white shadow-[0_35px_100px_#1b3b3017]">
        <div className="relative overflow-hidden bg-[#173f34] px-6 py-10 text-center text-white sm:px-10 sm:py-12"><div className="absolute left-[-70px] top-[-90px] size-64 rounded-full border-[45px] border-white/5" /><div className="absolute bottom-[-120px] right-[-70px] size-80 rounded-full border-[55px] border-[#dff0a7]/10" /><div className="relative"><span className="mx-auto grid size-20 place-items-center rounded-[26px] bg-[#dff0a7] text-[#315223] shadow-xl"><Check className="size-10" strokeWidth={3} /></span><p className="mt-6 text-xs font-black uppercase tracking-[.14em] text-[#dff0a7]">{hi ? "आपका job-ready pack तैयार है" : "Your job-ready pack is complete"}</p><h1 className="mt-3 font-[family-name:var(--font-display)] text-4xl font-bold tracking-[-.045em] sm:text-6xl">{hi ? "अब अगला कदम आपके हाथ में है।" : "You’re ready for the next step."}</h1><p className="mx-auto mt-4 max-w-xl text-[15px] leading-7 text-white/70">{hi ? `${profile.name}, आपकी कहानी अब काम की तैयारी बन गई है।` : `${profile.name}, your story is now a practical job-ready pack.`}</p><div className="mx-auto mt-7 grid max-w-3xl grid-cols-2 gap-2 sm:grid-cols-4">{[
          { icon: UserRound, en: "Profile ready", hi: "प्रोफ़ाइल तैयार" },
          { icon: BriefcaseBusiness, en: "Matches found", hi: "अवसर मिले" },
          { icon: FileText, en: "Resume ready", hi: "रिज़्यूमे तैयार" },
          { icon: GraduationCap, en: "Practice ready", hi: "अभ्यास तैयार" },
        ].map((step) => { const Icon = step.icon; return <div key={step.en} className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/7 px-3 py-2.5 text-left"><span className="grid size-7 shrink-0 place-items-center rounded-lg bg-[#dff0a7] text-[#315223]"><Icon className="size-3.5" /></span><span className="text-[11px] font-black text-white/85">{hi ? step.hi : step.en}</span></div>; })}</div><p className="mt-4 text-[11px] text-white/50">{hi ? "कोई आवेदन अपने-आप नहीं किया गया।" : "No application was submitted automatically."}</p></div></div>
