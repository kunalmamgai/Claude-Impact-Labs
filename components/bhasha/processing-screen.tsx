...
import { AlertCircle, ArrowDown, ArrowLeft, BookOpenCheck, Check, MapPin, Mic, RefreshCw, ShieldCheck, Sparkles, UserRound, Wrench } from "lucide-react";
...
export function ProcessingScreen({ language, error, onComplete, onRetry, onBack }: { language: AppLanguage; error?: string; onComplete: () => void; onRetry: () => void; onBack: () => void }) {
  ...
  return (
    <div className="mx-auto flex min-h-[calc(100vh-72px)] w-full max-w-5xl items-center justify-center px-4 pb-28 pt-8 sm:px-7">
      <section className="grid w-full overflow-hidden rounded-[32px] border border-[#d6e1d7] bg-white shadow-[0_35px_100px_#1a3a2f17] md:grid-cols-[.9fr_1.1fr]">
        <div className="flex flex-col justify-between bg-[#173f34] p-8 text-white sm:p-11">
          <div><span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-2 text-xs font-black"><Sparkles className="size-4 text-[#dff09f]" />BhashaHire intelligence</span><h1 className="mt-7 font-[family-name:var(--font-display)] text-4xl font-bold leading-tight tracking-[-.04em] sm:text-5xl">{hi ? "आपकी कहानी से करियर प्रोफ़ाइल" : "Your story becomes a career profile"}</h1><p className="mt-4 text-[15px] leading-7 text-white/70">{hi ? "आपके शब्द → साफ़, बदलने योग्य जानकारी। कुछ भी बढ़ा-चढ़ाकर नहीं।" : "Your words → clear, editable information. Nothing invented or overstated."}</p></div>
          <div className="mt-12 rounded-2xl border border-white/10 bg-white/5 p-4 text-xs leading-5 text-white/65"><ShieldCheck className="mr-2 inline size-4 text-[#dff09f]" />{hi ? "Matching में जाति, धर्म या लिंग नहीं।" : "No caste, religion or gender in matching."}</div>
        </div>
        <div className="p-7 sm:p-10">
          <div className="mb-5 flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-[.12em] text-[#648175]">{hi ? "आपकी बात समझ रहे हैं" : "Translating your story"}</p><p className="mt-1 text-sm font-bold text-[#65766f]">{Math.round(((active + 1) / tasks.length) * 100)}% {hi ? "पूरा" : "complete"}</p></div><div className="grid size-12 place-items-center rounded-2xl bg-[#e6f0ba] font-black text-[#355425]">AI</div></div>
          <Progress value={((active + 1) / tasks.length) * 100} className="mb-6 h-2.5 bg-[#e6ece7] [&_[data-slot=progress-indicator]]:bg-[#196b4f]" />
          {error ? (
            <div className="rounded-2xl border border-[#e8c9c3] bg-[#fff4f1] p-5" role="alert">
              <p className="flex items-center gap-2 font-black text-[#963f35]"><AlertCircle className="size-5" />{hi ? "प्रोफ़ाइल नहीं बन पाई" : "Profile could not be created"}</p>
              <p className="mt-2 text-sm leading-6 text-[#7a554e]">{error}</p>
              <div className="mt-4 flex flex-wrap gap-2"><Button onClick={onRetry} className="rounded-xl bg-[#196b4f] font-black"><RefreshCw />{hi ? "फिर कोशिश करें" : "Retry"}</Button><Button onClick={onBack} variant="outline" className="rounded-xl border-[#d8bdb7] font-black"><ArrowLeft />{hi ? "Transcript पर वापस" : "Back to transcript"}</Button></div>
            </div>
          ) : (
