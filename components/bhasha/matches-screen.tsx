...
import { ArrowRight, BookOpenCheck, BriefcaseBusiness, ChevronRight, CircleAlert, GraduationCap, IndianRupee, MapPin, Search, Sparkles } from "lucide-react";
...
export function MatchesScreen({ language, profile, matches, onOpen, onPrepare, onResume }: { language: AppLanguage; profile: CandidateProfile; matches: OpportunityMatch[]; onOpen: (match: OpportunityMatch) => void; onPrepare: (match: OpportunityMatch) => void; onResume: () => void }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[1320px] px-4 pb-28 pt-8 sm:px-7 lg:px-12 lg:pt-10">
      <PageTitle eyebrow={hi ? "कदम 3 / 4 · पात्रता के आधार पर" : "Step 3 of 4 · Eligibility first"} title={hi ? "ऐसे अवसर जिन पर आप आगे बढ़ सकते हैं" : "Opportunities you can act on"} description={hi ? "सिर्फ relevance नहीं—हम हर ज़रूरी शर्त को आपकी प्रोफ़ाइल से जाँचते हैं। हर अवसर हमारे demo snapshot से है और उसका source row दिया गया है।" : "Not just relevance—we check every hard requirement against your profile. Every result comes from our demo snapshot and cites its source row."} action={<Button onClick={onResume} variant="outline" className="min-h-11 rounded-xl border-[#cad8cd] bg-white font-black text-[#294b41]"><Sparkles />{hi ? "रिज़्यूमे बनाएँ" : "Build resume"}</Button>} />
      ...
      <Tabs defaultValue="eligible" className="gap-5">
        ...
        <TabsContent value="courses"><div className="grid gap-4 md:grid-cols-2">{relevantCourses.length ? relevantCourses.map((course) => <CourseCard key={course.id} language={language} course={course} />) : <div className="md:col-span-2">{emptyCopy}</div>}</div></TabsContent>
      </Tabs>
    </div>
  );
}
