...
export async function POST(request: Request) {
  ...
  try {
    ...
    return new Response(pdf, { headers: { "content-type": "application/pdf", "content-disposition": `attachment; filename="${(profile.data.name || "candidate").replace(/[^a-z0-9]+/gi, "-").toLowerCase()}-resume.pdf"`, "cache-control": "no-store" } });
  } catch (error) {
