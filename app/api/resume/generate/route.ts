...
import devanagariFontUrl from "@fontsource/noto-sans-devanagari/files/noto-sans-devanagari-devanagari-400-normal.woff?url";
import latinFontUrl from "@fontsource/noto-sans-devanagari/files/noto-sans-devanagari-latin-400-normal.woff?url";
...
async function renderPdf(profile: ReturnType<typeof userProfileSchema.parse>, language: string, origin: string) {
  const [latinResponse, devanagariResponse] = await Promise.all([fetch(new URL(latinFontUrl, origin)), fetch(new URL(devanagariFontUrl, origin))]);
  if (!latinResponse.ok || !devanagariResponse.ok) throw new Error("Resume font asset unavailable");
  const document = await PDFDocument.create();
  document.registerFontkit(fontkit);
  document.setTitle(`${profile.name || "Candidate"} — BhashaHire Resume`);
  document.setAuthor("BhashaHire");
  const latinFont = await document.embedFont(await latinResponse.arrayBuffer(), { subset: true });
  const devanagariFont = await document.embedFont(await devanagariResponse.arrayBuffer(), { subset: true });
...
export async function POST(request: Request) {
  ...
  try {
    const pdf = await renderPdf(profile.data, language, new URL(request.url).origin);
    const body = new ArrayBuffer(pdf.byteLength);
    new Uint8Array(body).set(pdf);
    return new Response(body, { headers: { "content-type": "application/pdf", "content-disposition": `attachment; filename="${(profile.data.name || "candidate").replace(/[^a-z0-9]+/gi, "-").toLowerCase()}-resume.pdf"`, "cache-control": "no-store" } });
  } catch (error) {
